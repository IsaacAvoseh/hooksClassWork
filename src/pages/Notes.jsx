...
function Notes() {
  const [payload, setPayload] = useState(localStorage.getItem('payload')? JSON.parse(localStorage.getItem('payload')): data1);
  ...
  const handleTag = (index, chooseTag)=>{
    ...
    allTags.map((item, id) =>{
      if(id===index){
        if (item.tag === chooseTag) {
          item.tag = "nav";
          console.log('Tag is', chooseTag)
          setWatch(!watch);
        } else {
          item.tag = chooseTag;
          console.log('Default Tag is', chooseTag)
          setWatch(!watch);
        }
      }
    })
 ...
 const saveNote = () => {
   ...
   setPayload(updatedPayload)
    console.log("new payload", updatedPayload);
    setShow(false)
    localStorage.setItem("payload", JSON.stringify(updatedPayload));
  };
//delete and remove from local storage
  const deleteNote = (index) => {
    console.log("Deleting")
    const updatedPayload = payload.filter((item, i) => i !== index);
    // const copy = [...payload]
    setPayload(updatedPayload);
    localStorage.setItem("payload", JSON.stringify(updatedPayload));
  };
                  ...
                  <div className="app-note-container">
                    ...
                    <div id="ct" className="note-container note-grid">
                      ...
                      {payload.map((item, index) => (
                        <div className={`note-item all-notes note-${item.tag}`}>
